...
export function EstadoVazio({ titulo, texto, children, className }: Props) {
  return (
    <div
      className={cn(
        "flex flex-1 flex-col items-center justify-center gap-2.5 text-center text-sub [&>button]:shrink-0",
        className
      )}
    >
      {titulo && <h2 className="m-0 font-titulo text-4xl font-semibold tracking-[-0.01em] text-ink">{titulo}</h2>}
