...
const TAMANHO_TITULO: Record<number, string> = { 1: "text-4xl", 2: "text-[19px]", 3: "text-[17px]" };
...
>(function LiveMdEditor({ value, onChange, placeholder, colapsoKey, cru }, ref) {
  ...
  return (
    <div
      className={cn(CORPO, "cursor-text font-sans")}
      onClick={(e) => {
      ...
      {linhas.map((linha, i) =>
        i !== ativa && ocultaPor[i] != null ? null : i === ativa ? (
          <textarea
            key={i}
            ref={taRef}
            className="m-0 block w-full resize-none overflow-hidden border-0 bg-transparent p-0 leading-[inherit] text-ink [font:inherit] focus:outline-none"
            rows={1}
            value={linha}
          ...
          <div key={i} className="min-h-[1.65em] [overflow-wrap:anywhere]" onClick={(e) => tocarLinha(e, i)}>
            {vazio ? (
              ...
              <LinhaMd
                ...
                toggle={
                  chaves[i]
                    ? {
                        aberto: !recolhidos.has(chaves[i]!),
                        ocultas: escondidas.get(i) || 0,
                        onToggle: () => alternarTitulo(i),
                      }
                    : undefined
                }
              />
...
function LinhaMd({
  ...
  if (l.type === "heading")
    return (
      <span className={cn("flex items-center gap-1 font-titulo font-bold", TAMANHO_TITULO[l.level])}>
        {toggle && (
          <button
            type="button"
            className="-ml-1 inline-flex size-6 flex-none cursor-pointer items-center justify-center rounded-xs border-0 bg-transparent p-0 text-sub [@media(any-hover:hover)]:hover:bg-card-2 [@media(any-hover:hover)]:hover:text-ink"
  ...
  if (l.type === "bullet" || l.type === "ordered")
    return (
      <span className="flex items-start gap-2">
        <span className={l.type === "ordered" ? "min-w-[1.2em] text-sub tabular-nums" : undefined}>
          {l.type === "ordered" ? l.marker : "•"}
        </span>
        <span>
          <Negrito text={l.text} />
        </span>
