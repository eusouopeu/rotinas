...
Antes de implementar, escolha 2–3 testes essenciais para a mudança. Depois, rode testes da área; mudança no `index.html` exige `npm test`. Não leia dependências/artefatos para formular contexto. `npm test` inclui legado e React; mudança visual no `webapp/` também exige `npm run visual` (regressão visual; ver `design-system.md`); use testes específicos como `test/editor.cjs`, `test/gamificacao.cjs`, sync e Vitest quando relevantes.
