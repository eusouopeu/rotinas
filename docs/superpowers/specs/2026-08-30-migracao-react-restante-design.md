...
3. Agenda: calendário mensal (`calendarioMesHtml`), compromissos avulsos, eventos iCal, adiar bloco por swipe/arrastar.
4. Mini player.
5. Simulação de pontuação.
6. MCP: handlers de tool (`list_routines`, `append_diario` etc.) — depende do kanban do Diário e de outras coleções ainda não portadas.
...
3. **Agenda completa** (calendário mensal, compromissos avulsos, iCal, adiar bloco) — depende do DnD consolidado para adiar por arrastar.
...
6. **MCP (handlers de tool)** — depende do kanban do Diário; entra só quando a Agenda (fase 3) e o Diário tiverem paridade suficiente.
...
| Agenda completa | `gsd-quick` com plano curto inline; `react-migration` (skill do projeto) para o procedimento de port; `context-mode` para explorar `agenda.ts`/trechos do legado sem `Read` bruto |
...
- MCP handlers dependem de coleções ainda não portadas (kanban do Diário) — a ordem no sequenciamento assume que essa dependência é resolvida na fase de Agenda; se não for, MCP fica bloqueado e a fase de corte final adia.
