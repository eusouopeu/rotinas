...
- Editor de notas: título e data ficam na barra do topo (editáveis), com excluir (vermelho); o voltar é ícone solto, sem moldura. Barra de baixo: B, lista, numerada, por letra, checklist, diminuir/aumentar recuo, tabela | modo cru (`</>`), arquivar. Copiar-tudo e concluir edição foram REMOVIDOS a pedido do Pedro em 22/09/2026 — não recriar. Itálico, título (H) e anexar imagem NÃO entram — decisão do Pedro em 22/09/2026.
- Editor de rotina (React): tem peso no boletim (Baixo/Médio/Alto) e área da roda da vida logo abaixo do nome. Os chips de área aparecem mesmo com a roda desligada. Aviso de vagas por nível segue só no legado.
